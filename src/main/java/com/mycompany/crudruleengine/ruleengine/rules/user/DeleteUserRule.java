...
public class DeleteUserRule implements ServiceRule{
    ...
    public JSONObject apply(Object input) {
        ...
        return(response.isSuccess())
                ?Util.buildResponse(Constants.SUCCESS_STATUS_CODE, ResultIds.USER_DELETED.name(), response.message())
                :Util.buildResponse(Constants.FAILURE_STATUS_CODE, ResultIds.USER_NOT_DELETED.name(), response.message());
