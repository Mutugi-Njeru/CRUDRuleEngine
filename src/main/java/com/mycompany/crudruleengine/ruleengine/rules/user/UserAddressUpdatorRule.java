 ...
 */
@ApplicationScoped
public class UserAddressUpdatorRule implements ServiceRule {
    ...
    public boolean matches(Object input) {
        return (input.toString().equalsIgnoreCase(RequestTypes.UPDATE_USER_ADDRESS.name()));
    }

    @Override
    public JSONObject apply(Object input) {
        JSONObject request=new JSONObject(input.toString());
        ServiceResponder response=userService.updateUserAddress(request);
        
        return(response.isSuccess())
                ?Util.buildResponse(Constants.SUCCESS_STATUS_CODE, ResultIds.ADDRESS_CHANGED.name(), response.message())
                :Util.buildResponse(Constants.FAILURE_STATUS_CODE, ResultIds.ADDRESS_NOT_CHANGED.name(), response.message()); 
    }
